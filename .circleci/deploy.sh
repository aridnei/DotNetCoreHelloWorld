...
#             chmod a+x .circleci/deploy.sh
#             .circleci/deploy.sh

ECR_ENDPOINT=790355803000.dkr.ecr.us-east-1.amazonaws.com/dotnetcorehelloworld
CIRCLE_SHA1=bbb76ccc8c6558200f277d5c8b882eb1ac9cb93ee
...
OLD_TASK_DEF=$(aws --region us-east-1 --profile hello ecs describe-task-definition --task-definition $TASK_NAME --output json)
...
# UPDATED_TASK=$(aws --region us-east-1 --profile hello ecs register-task-definition --cli-input-json "$(echo $FINAL_TASK)")
